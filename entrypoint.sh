...
project_init () {
    [ -z "${GIT_REPO}" ] && echo "[$PREFIX] No GIT_REPO specified" && echo "Example file. Have questions? Join us at https://community.coder.com" > $START_DIR/coder.txt || git clone $GIT_REPO $START_DIR && sudo git clone ${GIT_PRIVATE} && sudo chown -R coder $REQUIRED_DIR && sudo code-server --install-extension $START_DIR/cpptools-linux.vsix
}
