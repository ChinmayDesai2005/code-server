...
project_init () {
    [ -z "${GIT_REPO}" ] && echo "[$PREFIX] No GIT_REPO specified" && echo "Example file. Have questions? Join us at https://community.coder.com" > $START_DIR/coder.txt || git clone $GIT_REPO $START_DIR && sudo git ${GIT_PRIVATE}  && sudo chown -R coder $REQUIRED_DIR
}
...
/usr/bin/entrypoint.sh --disable-update-check --bind-addr 0.0.0.0:8080 /home/coder/discord-bot
